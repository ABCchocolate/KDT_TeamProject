...
public class MemberDAO {
	...
	public void insert(Member m) {
		...
		try {
			String sql = "INSERT INTO member(member_password, member_email, member_name, member_hiredate, dept_id, auth_id) VALUES (?, ?, ?, ?, ?, ?)";
			pstmt = con.prepareStatement(sql);
	...
	public void update(Member m) {
		...
		try {
			String sql = "UPDATE member SET member_password = ?, member_email = ?, member_name = ?, member_hiredate = ?, dept_id = ?, auth_id = ? WHERE member_id = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, m.getMember_password());
			pstmt.setString(2, m.getMember_email());
			pstmt.setString(3, m.getMember_name());
